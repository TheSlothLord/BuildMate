...
// Offers to install BuildMate to the home screen when opened in a mobile browser
// (native install prompt on Android, Safari instructions on iOS). Skipped in the
// Electron/Capacitor native builds and when already running standalone.
...
export function InstallPrompt() {
  ...
  const add = () => {
    ...
    else if (isIOS()) setIos(true);
  };

  if (!ready || isStandalone() || (!deferred && !isIOS())) return null;
